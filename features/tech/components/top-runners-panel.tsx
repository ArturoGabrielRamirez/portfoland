...
import type { Runner, TopRunnersPanelProps } from "@/features/tech/types/dashboard"
...
/**
 * Renders the global XP leaderboard with real runner data.
 * The current user's row is highlighted with a left cyan border and
 * a faint background tint (matching the spec).
 *
 * Data is fetched server-side in dashboard/page.tsx via getTopRunners
 * and passed directly — no hardcoded fallback data.
 */
export function TopRunnersPanel({ runners, className }: TopRunnersPanelProps) {
  const maxXP = runners.length > 0 ? Math.max(...runners.map((r) => r.xp)) : 1
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-[hsl(174,100%,50%,0.1)] bg-[hsl(200,30%,8%)]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono uppercase tracking-[0.2em] text-[hsl(52,100%,50%,0.8)]">
            Top Runners
          </span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {runners.length === 0 ? (
          <p className="text-[10px] font-mono text-muted-foreground/40 p-4">
            No runners yet.
          </p>
        ) : (
          runners.map((runner, i) => {
            ...
            const displayName = runner.username ?? runner.name
              ...
              <div
                ...
                className={cn(
                  ...
                  runner.isCurrentUser
                    ? "bg-[hsl(174,100%,50%,0.06)] border-l-2 border-[hsl(174,100%,50%)]"
                  ...
                  {runner.isCurrentUser && (
                    <div
                      ...
                      style={{
                        ...
                        boxShadow: "0 0 4px hsl(150,100%,45%)",
                      }}
                    />
                  )}
                </div>
