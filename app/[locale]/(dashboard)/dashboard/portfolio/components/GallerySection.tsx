...
export function GallerySection({ items, portfolioMode }: GallerySectionProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className={cn('text-sm font-semibold', mc.isTech ? 'font-mono text-foreground' : 'text-gray-900')}>Gallery</h3>
      ...
      {showForm && (
        <div className={cn(mc.card, 'p-4')}>
          <h4 className={cn(mc.subHeading, 'mb-3')}>{editingItem ? 'Edit Image' : 'New Image'}</h4>
          <div className="grid gap-3">
            ...
            {imageUrl && (
              <div className={cn('overflow-hidden w-40', mc.isTech ? 'rounded-sm border border-[hsl(174,100%,50%,0.15)]' : 'rounded-lg border border-gray-200')}>
                <img src={imageUrl} alt={altText || 'Preview'} className="h-28 w-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
              </div>
            )}
            <div>
            ...
            <div className="flex items-center gap-3">
              <input type="checkbox" id="gal-pub" checked={published} onChange={(e) => setPublished(e.target.checked)} className="w-4 h-4" />
              <label htmlFor="gal-pub" className={mc.label}>Published (visible on portfolio)</label>
            </div>
      ...
      {items.length === 0 && !showForm ? (
        <div className="text-center py-8">
          <Images className={cn('w-10 h-10 mx-auto mb-3', mc.isTech ? 'text-[hsl(174,100%,50%,0.3)]' : 'text-gray-300')} />
          <p className={cn('text-sm mb-3', mc.isTech ? 'font-mono text-muted-foreground' : 'text-gray-500')}>No gallery items yet</p>
          <button onClick={openCreate} className={cn(mc.primaryButton, 'mx-auto')}><Plus className="w-4 h-4" /> Add First Image</button>
        </div>
      ) : (
        <div className="grid gap-3">
          {items.map((item) => (
            <div key={item.id} className={cn(mc.itemCard, 'hover:shadow-sm transition-shadow')}>
              <div className="flex items-start gap-3 flex-1 min-w-0">
                <img src={item.imageUrl} alt={item.altText ?? ''} className={cn('h-16 w-24 object-cover shrink-0', mc.isTech ? 'rounded-sm bg-[hsl(200,20%,13%)]' : 'rounded-lg bg-gray-100')} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    {item.caption && <span className={cn('text-sm font-medium', mc.isTech ? 'font-mono text-foreground' : 'text-gray-900')}>{item.caption}</span>}
                    {item.category && (
                    ...
                    <span className={mc.badge(item.published)}>{item.published ? 'Live' : 'Draft'}</span>
                  </div>
                  <p className={cn('text-[11px] mt-1 truncate', mc.isTech ? 'font-mono text-muted-foreground' : 'text-gray-400')}>{item.imageUrl}</p>
                </div>
              </div>
