...
/**
 * CyberpunkTooltipWithSpotlights Component
 * 
 * Enhanced tooltip that controls static spotlights:
 * - Same cyberpunk tooltip design
 * - Snake/serpent effect controls external spotlights
 * - Left-to-right spotlight activation
 * - Synchronized with tooltip visibility
 */

import { useState, useRef, useEffect } from 'react'
import { cn } from '@/lib/utils'

interface CyberpunkTooltipWithSpotlightsProps {
...
export function CyberpunkTooltipWithSpotlights({
  children,
  content,
  position = 'top',
  color = 'cyan',
  ...
  onSpotlightActivate,
  onMouseLeave
}: CyberpunkTooltipWithSpotlightsProps) {
    ...
    >
      ...
      {isVisible && (
        ...
        >
          ...
          >
            {/* SNAKE EFFECT - Now controls external spotlights instead of internal effect */}
            <div 
              className="absolute inset-0 opacity-50"
              style={{
                background: `linear-gradient(90deg, 
                  transparent 0%, 
                  ${colors.snakeColor} 10%, 
                  transparent 20%, 
                  ${colors.snakeColor} 30%, 
                  transparent 40%, 
                  ${colors.snakeColor} 50%, 
                  transparent 60%, 
                  ${colors.snakeColor} 70%, 
                  transparent 80%, 
                  ${colors.snakeColor} 90%, 
                  transparent 100%)`,
                animation: 'snakeScan 2s linear infinite',
              }}
            />
      ...
      {/* Add animation styles - Snake effect now controls external spotlights */}
      <style jsx>{`
        @keyframes snakeScan {
          0% {
