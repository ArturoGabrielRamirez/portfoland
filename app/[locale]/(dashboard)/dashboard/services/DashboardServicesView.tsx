  ...
  return (
    <>
      ...
      <div className={cn('px-6 py-6 flex items-center justify-between', mc.headerBorder)}>
        <h1 className={mc.heading}>Services</h1>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
          ...
          <div className={cn(mc.card, 'p-6 mb-6')}>
            ...
            <div className="grid gap-4">
              ...
              <div className="flex items-center gap-3">
                <input type="checkbox" id="svc-published" checked={published} onChange={(e) => setPublished(e.target.checked)} className="w-4 h-4" />
                <label htmlFor="svc-published" className={mc.label}>Published (visible on portfolio)</label>
              </div>
        ...
        {services.length === 0 && !showForm ? (
          <div className="text-center py-16">
            <Briefcase className={cn('w-16 h-16 mx-auto mb-4', mc.isTech ? 'text-[hsl(174,100%,50%,0.3)]' : 'text-gray-300')} />
          ...
          <div className="grid gap-4">
            {services.map((service) => (
              <div key={service.id} className={cn(mc.itemCard, 'gap-4 hover:shadow-sm transition-shadow')}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className={cn('font-semibold', mc.isTech ? 'font-mono text-foreground' : 'text-gray-900')}>
                      {service.title}
                    </h3>
                    <span className={cn('text-xs', mc.isTech ? 'font-mono text-[hsl(174,100%,50%)]' : 'text-blue-600 font-medium')}>
                      {formatPrice(service)}
                    </span>
                    ...
                    <span className={mc.badge(service.published)}>
                      {service.published ? 'Live' : 'Draft'}
                    </span>
                  </div>
                  <p className={cn('text-xs mt-1 line-clamp-2', mc.isTech ? 'font-mono text-muted-foreground' : 'text-gray-500')}>
                    {service.description}
                  </p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => openEdit(service)} className={mc.editButton} title="Edit">
                    <Pencil className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(service)} disabled={isPending} className={cn(mc.dangerButton, 'disabled:opacity-50')} title="Delete">
