 ...
 * duration, cost badge, and external link. Includes broken link reporting.
 */
...
interface LearningResourceCardProps {
  ...
  onReport?: (url: string, type: LearningResource['type']) => void;
  isReporting?: boolean;
}

export function LearningResourceCard({ resource, onReport, isReporting }: LearningResourceCardProps) {
  return (
    <div className={cn(
      'rounded-sm bg-[#0D1117] border border-[#1E293B] transition-colors group',
      ...
      {onReport && (
        <div className="px-3 pb-2 flex items-center justify-between">
          <p className="text-xs text-[#475569]">⚠ AI-generated — verify before using</p>
          <button
            onClick={() => onReport(resource.url, resource.type)}
            disabled={isReporting}
            className={cn(
              'flex items-center gap-1 text-xs text-[#475569] hover:text-[#EF4444] transition-colors',
              'disabled:opacity-50 disabled:cursor-not-allowed'
            )}
            title="Report this link as broken"
          >
            {isReporting ? (
            ...
            {isReporting ? 'Getting replacement...' : 'Broken?'}
          </button>
        </div>
      )}
    </div>
